...
namespace DITI_LIVROM
{
    public partial class frm_tranferencia_plantao : Form
    {
            ...
            try
            {
                ...
                if (e_EDIT_ADD.Text == "ADD")
                    iInsertResult = Consulta.Executar("INSERT INTO tb_plantao(id_unidade, id_modulo, id_plantao, data_plantao_ini,LOCAL_DT) " +
                                                      ...
                                                      "Format('" + eDTPlantaoIni.Text + "', 'yyyy-mm-dd hh:nn:ss'), '" + eLOCAL_DT.Text + "'); ");
                //-------------------------------------
                //EDITAR PLANTAO - EDIT
                else { }
                //-------------------------------------
                ...
                if (iInsertResult > 0)
                {
                    ...
                    byte[] byteArray = new byte[eInstFi.Lines.Length];
                    byteArray = System.Text.Encoding.UTF8.GetBytes(eInstFi.Rtf);
                    byte[] byteArray1 = new byte[ePatrim.Lines.Length];
                    byteArray1 = System.Text.Encoding.UTF8.GetBytes(ePatrim.Rtf);
                    Globais.DB_Database.Open();
                    vCommand.Connection = Globais.DB_Database;
                    vSQL = "UPDATE tb_plantao SET txt_inst_fisic =  @texto_binary, " +
                    ...
                    vCommand.Parameters.AddWithValue("@texto_binary", byteArray);
                    vCommand.Parameters.AddWithValue("@texto_binary1", byteArray1);
                    iInsertResult = vCommand.ExecuteNonQuery();
                }
                //-------------------------------------
                ...
                if (iInsertResult > 0)
                {
                    foreach (ListViewItem itemRow in eJovens.Items)
                        if (itemRow.ImageIndex != 2)
                            Consulta.Executar("INSERT INTO tb_plantao_transf_internos(id_unidade, id_modulo, id_plantao, id_interno, LOCAL_DT) " +
                                              ...
                                              itemRow.SubItems[4].Text + "', '" + eLOCAL_DT.Text + "'); ");
                    foreach (ListViewItem itemRow in eServidores.Items)
                        Consulta.Executar("INSERT INTO tb_plantao_transf_servidor(id_unidade, id_modulo, id_plantao, cpf_servidor, LOCAL_DT) " +
                                          "VALUES( '" + Globais.Unidade + "', '" + Globais.Modulo + "', '" + Globais.Plantao + "', '" +
                                          itemRow.SubItems[1].Text + "', '" + eLOCAL_DT.Text + "'); ");
                }
        ...
        private void carregar_plantao(string _plantao)
        {
            ...
            try
            {
                string queryString = "SELECT tb_plantao.id_plantao, tb_plantao.txt_inst_fisic, tb_plantao.txt_patrimo FROM tb_plantao " +
                                     "WHERE(tb_plantao.id_unidade = '" + Globais.Unidade + "') AND (tb_plantao.id_modulo = '" + Globais.Modulo + "') " +
                                     "ORDER BY tb_plantao.data_plantao_ini DESC; ";
                ...
                Boolean bCarregacampos = false;
                while (reader.Read())
                {
                    if (reader.GetString(0) == Globais.Plantao)
                    {
                        bCarregacampos = true;
                        continue;
                    }
                    if (bCarregacampos)
                    {
                        var bytes = (byte[])reader.GetValue(1); // campo da tabela 2 - txt_ocorrencia
                        byte[] documento = new byte[0];
                        documento = bytes;
                        string originalRtf = System.Text.Encoding.UTF8.GetString(documento);
                        eInstFi.Rtf = originalRtf;

                        bytes = (byte[])reader.GetValue(2); // campo da tabela 2 - txt_ocorrencia
                        documento = new byte[0];
                        documento = bytes;
                        originalRtf = System.Text.Encoding.UTF8.GetString(documento);
                        ePatrim.Rtf = originalRtf;
                        break;
                    }
