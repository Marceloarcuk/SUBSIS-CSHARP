...
namespace DITI_LIVROM
{
    partial class frm_sel_servidor_todos : Form
    {
        ...
        private void buscar_servidores(object sender, EventArgs e)
        {
            ...
            try
            {
                string vSQL_UNID = "AND (coorporativo_tb_lotacao.id_unidade = '" + Globais.Unidade + "') ";
                if (buscar_unidades.SelectedIndex > 0) vSQL_UNID = "";

                RecordSet = Consulta.Consultar("SELECT corporativo_tb_servidor.id_cpf, corporativo_tb_servidor.nome_serv, coorporativo_tb_cargo.id_sys " +
                                                ...
                                                "WHERE (corporativo_tb_servidor.nome_serv Like '%' + '" + buscar_servidor.Text + "' + '%') " +
                                                 vSQL_UNID + " ORDER BY corporativo_tb_servidor.nome_serv; ");
        ...
        private void frm_sobre_jovem_Load(object sender, EventArgs e)
        {
            ...
            buscar_unidades.SelectedIndex = 0;
        }
