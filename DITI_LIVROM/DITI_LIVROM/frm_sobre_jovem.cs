    ...
    partial class frm_sobre_jovem : Form
    {
        ...
        private void frm_sobre_jovem_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                */
                RecordSet = Consulta.Consultar("SELECT tb_acao.desc_acao, tb_plantao_registro.txt_retorno, tb_plantao_registro.data_registro, Format([data_registro], 'dd /mm/yyyy') AS data_registro_data, Format([data_registro], 'hh:nn:ss') AS data_registro_hora, tb_plantao_registro_internos.id_interno " +
                                         "FROM(tb_acao INNER JOIN tb_plantao_registro ON tb_acao.id_acao = tb_plantao_registro.id_acao) INNER JOIN (tb_interno INNER JOIN tb_plantao_registro_internos " +
                                              "ON tb_interno.id_interno = tb_plantao_registro_internos.id_interno) ON(tb_plantao_registro.id_registro = tb_plantao_registro_internos.id_registro) " +
                                              "AND(tb_plantao_registro.id_plantao = tb_plantao_registro_internos.id_plantao) AND(tb_plantao_registro.id_modulo = tb_plantao_registro_internos.id_modulo) " +
                                              "AND(tb_plantao_registro.id_unidade = tb_plantao_registro_internos.id_unidade) " +
                                         "WHERE((tb_plantao_registro_internos.id_interno = '" + codigo.Text + "')  " +
                                         "  AND (tb_plantao_registro.id_plantao = '" + Globais.Plantao + "'))  " +
                                         "ORDER BY tb_plantao_registro.id_plantao DESC, tb_plantao_registro.id_registro DESC;  ");
