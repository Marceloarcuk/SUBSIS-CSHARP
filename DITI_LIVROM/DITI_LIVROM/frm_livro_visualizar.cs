            ...
            try
            {
                ...
                try
                {
                    vSQL = "SELECT tb_plantao.id_unidade, tb_plantao.id_modulo, tb_plantao.id_plantao, tb_plantao.data_plantao_fim, tb_plantao.txt_inst_fisic, tb_plantao.txt_patrimo, " +
                                  "tb_plantao.cpf_servidor_passagem, corporativo_tb_servidor.nome_serv as nome_serv_passagem, corporativo_tb_servidor.matricula_serv as matricula_serv_passagem, " +
                                  "tb_plantao.cpf_servidor_recebime, corporativo_tb_servidor_1.nome_serv as nome_serv_recebime, corporativo_tb_servidor_1.matricula_serv as matricula_serv_recebime, tb_plantao.LOCAL_DT, tb_plantao.SERVER_DT " +
                    ...
                    foreach (DataRow pRow in vRecordSet.Tables[0].Rows)
                    {
                        vServRecebeNome = pRow.Field<string>("nome_serv_recebime");      // 10
                        vServRecebeMatr = pRow.Field<string>("matricula_serv_recebime"); // 11
                        vServTransfNome = pRow.Field<string>("nome_serv_passagem");      // 7
                        vServTransfMatr = pRow.Field<string>("matricula_serv_passagem"); // 8
                        try { vDataHoraRecebe = pRow.Field<DateTime>("data_plantao_fim"); bDataHoraRecebeVerif = true; } catch { bDataHoraRecebeVerif = false; } // 3
                        try { txtInstalacaoFisica = Globais.ByteArrayToString((Byte[])pRow["txt_inst_fisic"]); } catch { } // 4
                        try { txtPatrimonio = Globais.ByteArrayToString((Byte[])pRow["txt_patrimo"]); } catch { }          // 5
                    }
                }
                catch (Exception ex)
                {
