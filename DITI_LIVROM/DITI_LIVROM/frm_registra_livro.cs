...
{
    public partial class frm_registra_livro : Form
    {
        public frm_registra_livro()
        {
            InitializeComponent();
        }
                ...
                if (iInsertResult > 0)
                {
                    ...
                    foreach (ListViewItem itemRow in eJovens.Items)
                        Consulta.Executar("UPDATE tb_unidade_interno SET status_interno = " + eAcao.Text + " " +
                                          "WHERE(id_unidade = '" + Globais.Unidade + "') " +
                                           " AND(id_modulo = '" + Globais.Modulo + "') " +
                                           " AND (id_interno = '" + itemRow.SubItems[1].Text + "');", "", null, "", null);
                }
                else
                {
        ...
        private void frm_registra_livro_Load(object sender, EventArgs e)
        {
